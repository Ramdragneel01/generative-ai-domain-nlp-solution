...
def _build_rate_limiter(runtime_settings: Settings) -> RateLimiter:
    """Build requested rate limiter backend from runtime configuration."""

    if runtime_settings.rate_limit_backend == "redis":
        return RedisRateLimiter.from_url(
            runtime_settings.redis_url,
            window_seconds=60,
            key_prefix=runtime_settings.rate_limit_redis_key_prefix,
            socket_timeout_seconds=runtime_settings.redis_socket_timeout_seconds,
        )
...
def _require_request_auth(
    ...
    """Validate gateway auth first, then optional API key for scoring endpoints."""

    if settings.gateway_auth_enabled:
        gateway_auth_value = request.headers.get(settings.gateway_auth_header, "")
        if not hmac.compare_digest(gateway_auth_value, settings.gateway_auth_secret):
            raise HTTPException(status_code=401, detail="Gateway authentication failed")

        if settings.require_gateway_principal and not request.headers.get(settings.gateway_principal_header):
            raise HTTPException(status_code=401, detail="Gateway principal header missing")
...
AuthDep = Annotated[None, Depends(_require_request_auth)]


def _enforce_rate_limit(request: Request) -> None:
    """Apply configured rate limit backend and map backend failures to 503."""

    try:
        allowed = limiter.allow(_client_key(request), settings.rate_limit_per_minute)
